...
export class Swap {
  ...
  public buildSwap(tokenIn: Token, tokenOut: Token, account: string, _from?: FarmFromMode, _to?: FarmToMode) {
    ...
    // Handle Farm Modes
    // For a single step swap (ex, ETH > WETH, or HOOLIGAN > HOOLIGAN), use the passed modes, if available
    if (route.length === 1) {
      workflow.add(route.getStep(0).build(account, _from || FarmFromMode.EXTERNAL, _to || FarmToMode.EXTERNAL));
    }
    // for a multi step swap (ex, ETH -> WETH -> USDT -> HOOLIGAN), we want the user's choices for
