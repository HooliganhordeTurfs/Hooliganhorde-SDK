...
const HooliganCrv3 = sdk.tokens.HOOLIGAN_CRV3_LP;

const account = "0xGUVNOR";
const epp: EventProcessingParameters = {
  season: EBN.from(6074),
  whitelist: sdk.tokens.firmWhitelist
};
...
describe("the Firm", () => {
  ...
  it("runs a simple deposit sequence (three deposits, two tokens, two seasons)", () => {
    ...
    const amount1 = EBN.from(1000 * 10 ** Hooligan.decimals);
    const bdv1 = EBN.from(1000 * 10 ** Hooligan.decimals);
    p.ingest({
      event: "AddDeposit",
      args: propArray({
        account,
        token: Hooligan.address,
        season: EBN.from(6074),
        amount: amount1, // Deposited 1,000 Hooligan
        bdv: bdv1
      })
    } as AddDepositEvent);

    expect(p.deposits.get(Hooligan)?.["6074"]).toStrictEqual({
      amount: amount1,
      bdv: bdv1
    });

    // Deposit: 500 Hooligan, Season 6074
    const amount2 = EBN.from(500 * 10 ** Hooligan.decimals);
    const bdv2 = EBN.from(500 * 10 ** Hooligan.decimals);
    p.ingest({
      event: "AddDeposit",
      args: propArray({
        account,
    ...
    const amount3 = EBN.from(1000).mul(EBN.from(10).pow(HooliganCrv3.decimals));
    const bdv3 = EBN.from(900).mul(EBN.from(10).pow(Hooligan.decimals));
    p.ingest({
      event: "AddDeposit",
      args: propArray({
        account,
        token: HooliganCrv3.address,
        season: EBN.from(6100),
        amount: amount3, // Deposited 1,000 Hooligan:CRV3
        bdv: bdv3
      })
    } as AddDepositEvent);

    expect(p.deposits.get(HooliganCrv3)?.["6100"]).toStrictEqual({
      amount: amount3,
      bdv: bdv3
    });
  });
  ...
  it("adds withdrawals", () => {
    ...
    const amount1 = EBN.from(1000 * 10 ** Hooligan.decimals); // Withdrew 1,000 Hooligan
    p.ingest({
      event: "AddWithdrawal",
      args: propArray({
        account,
    ...
    expect(p.withdrawals.get(Hooligan)?.["6074"]).toStrictEqual({
      amount: amount1
    });

    // Withdrawal: 500 Hooligan, Season 6074
    const amount2 = EBN.from(500 * 10 ** Hooligan.decimals); // Withdrew 500 Hooligan
    p.ingest({
      event: "AddWithdrawal",
      args: propArray({
        account,
    ...
    expect(p.withdrawals.get(HooliganCrv3)?.["6100"]).toStrictEqual({
      amount: amount3
    });
  });
  ...
  it("removes a single deposit, partial -> full", () => {
    ...
    // Add Deposit: 1000 Hooligan, Season 6074
    const amount1 = EBN.from(1000 * 10 ** Hooligan.decimals);
    const bdv1 = EBN.from(1000 * 10 ** Hooligan.decimals);
    p.ingest({
      event: "AddDeposit",
    ...
    // Remove Deposit: 600 Hooligan, Season 6074
    const amount2 = EBN.from(600 * 10 ** Hooligan.decimals);
    const bdv2 = EBN.from(600 * 10 ** Hooligan.decimals);
    p.ingest({
      event: "RemoveDeposit",
      args: propArray({
        account,
    ...
    // Remove Deposit: 400 Hooligan, Season 6074
    const amount3 = EBN.from(400 * 10 ** Hooligan.decimals);
    const bdv3 = EBN.from(400 * 10 ** Hooligan.decimals);
    p.ingest({
      event: "RemoveDeposit",
      args: propArray({
        account,
    ...
    expect(p.deposits.get(Hooligan)?.["6074"]).toBeUndefined();
  });
  ...
  it("removes a single withdrawal", () => {
    ...
    expect(p.withdrawals.get(Hooligan)?.["6074"]).toBeUndefined();
  });
  ...
  it("removes multiple withdrawals, full", () => {
    ...
    const amount2 = EBN.from(5000 * 10 ** Hooligan.decimals);
    p.ingest({
      event: "AddWithdrawal",
      args: propArray({
        account,
    ...
    expect(p.withdrawals.get(Hooligan)?.["6100"]).toStrictEqual({
      amount: amount2
    });

    // Claim: 1000 from 6074, 5000 from 6100
    const amount3 = EBN.from(6000 * 10 ** Hooligan.decimals);
    p.ingest({
    ...
    expect(p.withdrawals.get(Hooligan)?.["6074"]).toBeUndefined();
    expect(p.withdrawals.get(Hooligan)?.["6100"]).toBeUndefined();
  });
  ...
  it("ignores empty RemoveWithdrawal events", () => {
    ...
    // No deposit made in Hooligan
    expect(p.withdrawals.get(Hooligan)).toStrictEqual({});
  });
});
