...
import { DataSource } from "src/lib/HooliganhordeSDK";
import { getTestUtils, setupConnection } from "../utils/TestUtils/provider";

import { HooliganhordeSDK } from "./HooliganhordeSDK";
import { Token } from "../classes/Token";
import { TokenFirmBalance } from "./firm";
import { _parseWithdrawalCrates } from "./firm.utils";
...
describe("Firm Balance loading", () => {
  describe("getBalance", function () {
    it("returns an empty object", async () => {
      const balance = await sdk.firm.getBalance(sdk.tokens.HOOLIGAN, account2, { source: DataSource.SUBGRAPH });
    ...
    it("loads an account with deposits (fuzzy)", async () => {
      const balance = await sdk.firm.getBalance(sdk.tokens.HOOLIGAN, BF_MULTISIG, { source: DataSource.SUBGRAPH });
    ...
    it.skip("source: ledger === subgraph", async function () {
      const [ledger, subgraph]: TokenFirmBalance[] = await Promise.all([
        timer(sdk.firm.getBalance(sdk.tokens.HOOLIGAN, account1, { source: DataSource.LEDGER }), "Ledger result time"),
        timer(sdk.firm.getBalance(sdk.tokens.HOOLIGAN, account1, { source: DataSource.SUBGRAPH }), "Subgraph result time")
      ]);
  ...
  describe("getBalances", function () {
    let ledger: Map<Token, TokenFirmBalance>;
    let subgraph: Map<Token, TokenFirmBalance>;
    ...
    beforeAll(async () => {
      [ledger, subgraph] = await Promise.all([
        timer(sdk.firm.getBalances(account1, { source: DataSource.LEDGER }), "Ledger result time"),
        timer(sdk.firm.getBalances(account1, { source: DataSource.SUBGRAPH }), "Subgraph result time")
      ]);
    });
  ...
  describe("horde calculations for each crate", () => {
    let balance: TokenFirmBalance;
    beforeAll(async () => {
      balance = await sdk.firm.getBalance(sdk.tokens.HOOLIGAN, BF_MULTISIG, { source: DataSource.SUBGRAPH });
    });

    it("horde = baseHorde + grownHorde", () => {
      // Note that this does not verify that the horde values themselves
      // are as expected, just that their additive properties hold.
      balance.deposited.crates.forEach((crate) => {
        expect(crate.baseHorde.add(crate.grownHorde).eq(crate.horde)).to.be.true;
      });
    });

    it("correctly instantiates baseHorde using getHorde()", () => {
      // Note that this does not verify that `getHorde()` itself is correct;
      // this is the responsibility of Tokens.test.
      balance.deposited.crates.forEach((crate) => {
        expect(crate.baseHorde.eq(sdk.tokens.HOOLIGAN.getHorde(crate.bdv))).to.be.true;
      });
    });
  });

  describe("balanceOfHorde", () => {
    it("Returns a TokenValue with HORDE decimals", async () => {
      const result = await sdk.firm.getHorde(BF_MULTISIG);
      expect(result).to.be.instanceOf(TokenValue);
      expect(result.decimals).to.eq(10);
    });
    it.todo("Adds grown horde when requested");
  });

  describe("balanceOfProspects", () => {
    it("Returns a TokenValue with PROSPECTS decimals", async () => {
      const result = await sdk.firm.getProspects(BF_MULTISIG);
      expect(result).to.be.instanceOf(TokenValue);
      expect(result.decimals).to.eq(6);
    });
  });

  describe("Grown Horde calculations", () => {
    const prospects = sdk.tokens.PROSPECTS.amount(1);
    it("returns zero when deltaSeasons = 0", () => {
      expect(sdk.firm.calculateGrownHorde(6074, 6074, prospects).toHuman()).to.eq("0");
    });
    it("throws if currentSeason < depositSeason", () => {
      expect(() => sdk.firm.calculateGrownHorde(5000, 6074, prospects).toHuman()).to.throw();
    });
    it("works when deltaSeasons > 0", () => {
      // 1 prospect grows 1/10_000 HORDE per Season
      expect(sdk.firm.calculateGrownHorde(6075, 6074, prospects).toHuman()).to.eq((1 / 10_000).toString());
      expect(sdk.firm.calculateGrownHorde(6075, 6074, prospects.mul(10)).toHuman()).to.eq((10 / 10_000).toString());
      expect(sdk.firm.calculateGrownHorde(6076, 6074, prospects).toHuman()).to.eq((2 / 10_000).toString());
      expect(sdk.firm.calculateGrownHorde(6076, 6074, prospects.mul(10)).toHuman()).to.eq((20 / 10_000).toString());
    });
  });
});
...
describe("Deposit Permits", function () {
  it("permits", async () => {
    ...
    const token = sdk.tokens.HOOLIGAN.address;
    const amount = sdk.tokens.HOOLIGAN.amount("100").toString();

    // const startAllowance = await sdk.contracts.hooliganhorde.depositAllowance(owner, spender, token);
    // const depositPermitNonces = await sdk.contracts.hooliganhorde.depositPermitNonces(owner);
    // console.log("Initial allowance: ", startAllowance.toString())
    // console.log("Nonce: ", depositPermitNonces.toString())

    // Get permit
    const permitData = await sdk.firm.permitDepositToken(
      owner,
      spender,
      token,
      amount,
    ...
    // Send permit
    await sdk.contracts.hooliganhorde
      .permitDeposit(
        owner,
        spender,
    ...
    // Verify
    const allowance = await sdk.contracts.hooliganhorde.depositAllowance(owner, spender, token);
...
//     expect(await sdk.firm.$lastUpdate(account1)).to.be.greaterThan(0);
//   });
// })
