  ...
  async run(_amountInStep: ethers.BigNumber, context: RunContext) {
    ...
    return {
      ...
      prepare: () => {
        ...
        return {
          target: UnwrapEth.sdk.contracts.hooliganhorde.address,
          callData: UnwrapEth.sdk.contracts.hooliganhorde.interface.encodeFunctionData("unwrapEth", [
            _amountInStep, // ignore minAmountOut since there is no slippage
            this.fromMode
          ])
        };
      },
      decode: (data: string) => UnwrapEth.sdk.contracts.hooliganhorde.interface.decodeFunctionData("unwrapEth", data),
      decodeResult: (result: string) => UnwrapEth.sdk.contracts.hooliganhorde.interface.decodeFunctionResult("unwrapEth", result)
    };
  }
}
