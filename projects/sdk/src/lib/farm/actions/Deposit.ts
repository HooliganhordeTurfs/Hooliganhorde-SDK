import { BasicPreparedResult, RunContext, Step, StepClass } from "src/classes/Workflow";
import { ethers } from "ethers";
import { FarmFromMode, FarmToMode } from "../types";
import { Token } from "src/classes/Token";

export class Deposit extends StepClass<BasicPreparedResult> {
  public name: string = "deposit";

  constructor(private token: Token, private fromMode: FarmFromMode = FarmFromMode.INTERNAL_EXTERNAL) {
    super();
  }

  async run(_amountInStep: ethers.BigNumber, context: RunContext) {
    Deposit.sdk.debug(`[${this.name}.run()]`, {
      token: this.token.symbol,
      amountInStep: _amountInStep,
      fromMode: this.fromMode,
      context
    });
    return {
      name: this.name,
      amountOut: _amountInStep,
      prepare: () => {
        Deposit.sdk.debug(`[${this.name}.prepare()]`, {
          token: this.token.symbol,
          amountInStep: _amountInStep,
          fromMode: this.fromMode,
          context
        });
        if (!_amountInStep) throw new Error("Deposit: Missing _amountInStep");
        return {
          target: Deposit.sdk.contracts.hooliganhorde.address,
          callData: Deposit.sdk.contracts.hooliganhorde.interface.encodeFunctionData("deposit", [
            this.token.address,
            _amountInStep,
            this.fromMode
          ])
        };
      },
      decode: (data: string) => Deposit.sdk.contracts.hooliganhorde.interface.decodeFunctionData("deposit", data),
      decodeResult: (result: string) => Deposit.sdk.contracts.hooliganhorde.interface.decodeFunctionResult("deposit", result)
    };
  }
}
