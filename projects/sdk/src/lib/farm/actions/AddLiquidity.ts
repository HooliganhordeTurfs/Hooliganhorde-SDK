  ...
  async run(_amountInStep: ethers.BigNumber, context: RunContext): Promise<Step<BasicPreparedResult>> {
    ...
    return {
      ...
      prepare: () => {
        ...
        return {
          target: AddLiquidity.sdk.contracts.hooliganhorde.address,
          callData: AddLiquidity.sdk.contracts.hooliganhorde.interface.encodeFunctionData("addLiquidity", [
            this._pool,
            this._registry,
      ...
      decode: (data: string) => AddLiquidity.sdk.contracts.hooliganhorde.interface.decodeFunctionData("addLiquidity", data),
      decodeResult: (result: string) => AddLiquidity.sdk.contracts.hooliganhorde.interface.decodeFunctionResult("addLiquidity", result)
    };
  }
}
