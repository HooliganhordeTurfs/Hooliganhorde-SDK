  ...
  async run(_amountInStep: ethers.BigNumber, context: RunContext) {
    ...
    return {
      ...
      prepare: () => {
        ...
        return {
          target: Exchange.sdk.contracts.hooliganhorde.address,
          callData: Exchange.sdk.contracts.hooliganhorde.interface.encodeFunctionData("exchange", [
            this.pool,
            this.registry,
            tokenIn.address,
            tokenOut.address,
      ...
      decode: (data: string) => Exchange.sdk.contracts.hooliganhorde.interface.decodeFunctionData("exchange", data),
      decodeResult: (result: string) => Exchange.sdk.contracts.hooliganhorde.interface.decodeFunctionResult("exchange", result)
    };
  }
}
