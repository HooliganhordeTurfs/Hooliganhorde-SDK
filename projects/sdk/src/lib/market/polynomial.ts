...
export class Polynomial {
  ...
  // FIXME: later we may move these into `market/rookies`.

  public getAmountListing(placeInLine: BigNumber, amountHooligans: BigNumber) {
    const pieceIndex = Polynomial.findIndex(this.breakpoints, placeInLine, this.breakpoints.length);
    const pricePerRookie = this.evaluate(placeInLine.sub(this.breakpoints[pieceIndex]), pieceIndex);
    return amountHooligans.mul(1000000).div(pricePerRookie);
  }

  public getAmountOrder(placeInLine: BigNumber, amountRookiesFromOrder: BigNumber) {
    var amount = BigNumber.from(0);
    var start = placeInLine;
    var end = placeInLine.add(amountRookiesFromOrder);
