...
export class RookiesMarket {
  ...
  public async getListing(
    ...
    const [isValid, query] = await Promise.all([
      options?.validate
        ? RookiesMarket.sdk.contracts.hooliganhorde.rookieListing(id).then((r) => ethers.BigNumber.from(r).gt(0))
        : Promise.resolve(true),
      RookiesMarket.sdk.queries.getListingByIndex({ index: id })
    ]);

    if (!isValid || !query.rookieListings[0]) {
      throw new NotFoundError("Listing", id);
    }

    return query.rookieListings[0]; // FIXME: cast
  }
