...
export class DepositOperation {
  ...
  constructor(sdk: HooliganhordeSDK, router: Router, targetToken: Token, account: string) {
    if (!sdk.tokens.firmWhitelist.has(targetToken)) throw new Error(`Cannot deposit ${targetToken.symbol}, not on whitelist.`);
  ...
  buildWorkflow() {
    const route = this.router.getRoute(this.inputToken.symbol, `${this.targetToken.symbol}:FIRM`);
  ...
  async getSummary() {
    ...
    const depositBDV = await DepositOperation.sdk.hooligan.getBDV(toToken, toToken.fromBlockchain(depositStep.amountOut));
    ...
    summary.push({
      ...
      horde: toToken.getHorde(depositBDV),
      prospects: toToken.getProspects(depositBDV)
    });
