...
type Reconfigurable = Pick<HooliganhordeConfig, "source">;
...
export class HooliganhordeSDK {
  ...
  public readonly hooligan: Hooligan;

  constructor(config?: HooliganhordeConfig) {
    ...
    // Hooliganhorde
    this.hooligan = new Hooligan(this);
  ...
  handleConfig(config: HooliganhordeConfig = {}) {
    if (config.rpcUrl) {
  ...
  deriveConfig<T extends HooliganhordeConfig>(key: keyof Reconfigurable, _config?: T): HooliganhordeConfig[typeof key] {
