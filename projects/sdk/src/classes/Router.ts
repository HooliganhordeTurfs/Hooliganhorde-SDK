...
export class Router {
  ...
  constructor(sdk: HooliganhordeSDK, graph: Graph, selfEdgeBuilder: SelfEdgeBuilder) {
    Router.sdk = sdk;
    this.graph = graph;
  ...
  public getRoute(tokenIn: string, tokenOut: string): Route {
    ...
    // for ex, swap HOOLIGAN to HOOLIGAN, or deposit HOOLIGAN to HOOLIGAN.
  ...
  getGraphCode() {
    ...
    Router.sdk.tokens.firmWhitelist.forEach((t) => {
      code += `\t"${t.symbol}:FIRM" [fillColor="#afb8ff" style="filled"]\n`;
    });
