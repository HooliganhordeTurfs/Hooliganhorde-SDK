...
import type { HooliganhordeSDK } from "../../lib/HooliganhordeSDK";
...
export abstract class Token {
  ...
  /** The Hooliganhorde HORDE/PROSPECT rewards per BDV of this token. */
  public readonly rewards?: { horde: TokenValue; prospects: TokenValue };
  ...
  public getHorde(bdv?: TokenValue): TokenValue {
    if (!this.rewards?.horde) return Token.sdk.tokens.HORDE.amount(0);
    if (!bdv) return this.rewards.horde;

    return this.rewards.horde.mul(bdv);
  }

  /** Get the amount of Prospects rewarded per deposited BDV of this Token. */
  public getProspects(bdv?: TokenValue): TokenValue {
    if (!this.rewards?.prospects) return Token.sdk.tokens.PROSPECTS.amount(0);
    if (!bdv) return this.rewards.prospects;

    return this.rewards.prospects.mul(bdv);
  }
   ...
   *
   * Ex: HOOLIGAN.fromHuman("3140000") => TokenValue holding value "3140000" and 6 decimals
   *
   ...
   *
   * Ex: HOOLIGAN.fromHuman("3.14") => TokenValue holding value "3140000" and 6 decimals
   *
   * @param amount human readable amout, ex: "3.14" ether
   * @returns TokenValue
   */
   ...
   *
   * Ex: HOOLIGAN.toHuman(BigNumber.from('3140000)) => "3.14"
   * @param value A BigNumber with a value of this token, for ex: 1000000 would be 1 HOOLIGAN
   * @returns string
   */
  ...
  public approveHooliganhorde(amount: TokenValue | BigNumber): Promise<ContractTransaction> {
    // @ts-ignore
    return;
  }
}
