...
export abstract class StepClass<P extends BasicPreparedResult = BasicPreparedResult> {
  ...
  public setSDK(sdk: HooliganhordeSDK) {
    StepClass.sdk = sdk;
  }
 ...
 * that involves multiple steps. This includes Hooliganhorde's `farm()` function,
 * the `pipeMulti` and `pipeAdvanced` functions provided by Pipeline and Clubhouse,
 * etc.
 *
 * ## BASICS
 *
...
> {
  ...
  constructor(sdk: HooliganhordeSDK, public name: string = "Workflow") {
    Workflow.sdk = sdk;
  }
   ...
   * For ex, if we are trading ETH -> HOOLIGAN, and you want to spend exactly 5 ETH, estimate()
   * would tell how much HOOLIGAN you'd receive for 5 ETH
   ...
   * For ex, if we are trading ETH -> Hooligan, and I want exactly 500 HOOLIGAN, estimateReversed()
   * tell me how much ETH will result in 500 HOOLIGAN
