import { HooliganhordeSDK, ERC20Token, FarmFromMode, Token, Clipboard, FarmToMode, TokenValue, Workflow } from "@hooligangturfs/sdk";
...
async function swap(sdk: HooliganhordeSDK, fromToken: Token, toToken: Token, _amount: number) {
  ...
  // Approval for Hooliganhorde contract to transfer "fromToken" from user to Pipeline
  await fromToken.approveHooliganhorde(TokenValue.MAX_UINT256).then((r) => r.wait());
  ...
  pipe.add(async function unloadPipeline(amountInStep, context) {
    // Approval for Hooliganhorde to transfer "toToken" from Pipeline to user
    await toToken.approveHooliganhorde(TokenValue.MAX_UINT256).then((r) => r.wait());
