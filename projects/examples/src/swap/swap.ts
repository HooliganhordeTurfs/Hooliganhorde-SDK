import { HooliganhordeSDK, ERC20Token, FarmFromMode, FarmToMode, Token } from "@hooligangturfs/sdk";
...
async function main() {
  const sdk = new HooliganhordeSDK({ signer, DEBUG: false });

  // await swap(sdk, sdk.tokens.USDT, sdk.tokens.USDT, "500", FarmFromMode.INTERNAL, FarmToMode.EXTERNAL);
  await swap(sdk, sdk.tokens.ETH, sdk.tokens.HOOLIGAN, "300");
  await swap(sdk, sdk.tokens.ETH, sdk.tokens.WETH, "1");

  // await swap(sdk, sdk.tokens.HOOLIGAN, sdk.tokens.ETH, "30");
...
async function swap(
  ...
  if (fromToken.symbol !== "ETH") {
    await (await (fromToken as ERC20Token).approve(sdk.contracts.hooliganhorde.address, amount.toBigNumber())).wait();
  }
