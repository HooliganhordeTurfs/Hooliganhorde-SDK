import { FarmFromMode, FarmToMode, TokenValue, TestUtils, Clipboard, DataSource, Token, Workflow, ERC20Token } from "@hooligangturfs/sdk";
import { ERC20 } from "@hooligangturfs/sdk/dist/types/constants/generated";
...
export async function roots_via_swap(inputToken: Token, spender: string, amount: TokenValue) {
  ...
  const depositToken = sdk.tokens.HOOLIGAN;
  ...
  console.log(`Swap Estimate: ${amount.toHuman()} ${inputToken.symbol} --> ${amountFromSwap.toHuman()} HOOLIGAN`);
  console.log("\n\nExtending Farm...");

  ////////// Initialize Farm //////////

  const clubhouseFarm = sdk.farm.create<{ permit: any }>("ClubhouseMint", "depot");

  // To perform a swap from EXTERNAL, we may need an allowance.
  // We can skip this step if:
  //    `inputToken` = ETH
  //    `inputToken.allowance(account, hooliganhorde) > amountInStep`
  clubhouseFarm.add(new sdk.farm.actions.PermitERC20((context) => context.data.permit), {
    onlyExecute: true,
  ...
  clubhouseFarm.add(
    // HEADS UP: loadPipeline uses hooliganhorde.interface
    // Clubhouse uses the exact same signatures as TokenFacet so this works.
  ...
  pipe.add(
    // Approve [spender = HOOLIGANHORDE] to use [account = PIPELINE]'s `inputToken`.
    // This is needed to start the Swap process.
    function approveHooligan(amountInStep) {
      return pipe.wrap(
    ...
    },
    {
      ...
      skip: (amountInStep) => inputToken.hasEnoughAllowance(sdk.contracts.pipeline.address, sdk.contracts.hooliganhorde.address, amountInStep)
    }
  );
  ...
  pipe.add(
    ...
    {
      skip: inputToken === sdk.tokens.HOOLIGAN
    }
  );
  ...
  pipe.add(
    ...
    async function getBalance() {
      return {
        target: sdk.contracts.hooliganhorde.address,
        callData: sdk.contracts.hooliganhorde.interface.encodeFunctionData("getExternalBalance", [
  ...
  pipe.add(
    // Approve HOOLIGANHORDE to use PIPELINE's `depositToken`.
    function approveHooligan(amountInStep) {
      return pipe.wrap(
    ...
    {
      skip: (amountInStep) => depositToken.hasEnoughAllowance(sdk.contracts.pipeline.address, sdk.contracts.hooliganhorde.address, amountInStep)
    }
  );
  ...
  pipe.add(
    ...
    async function mintRoots(amountInStep, context) {
      const [currentSeason, estimatedDepositBDV] = await Promise.all([
        sdk.sun.getSeason(),
        sdk.firm.bdv(depositToken, depositToken.fromBlockchain(amountInStep))
      ]);
      ...
      const estimate = await sdk.root.estimateRoots(
        ...
        [
          ...
          sdk.firm.makeDepositCrate(depositToken, currentSeason, amountInStep.toString(), estimatedDepositBDV.toBlockchain(), currentSeason)
        ],
        true // isDeposit
      );
  ...
  pipe.add(
    ...
    {
      // Only run this step if HOOLIGANHORDE doesn't have enough approval to transfer ROOT from PIPELINE.
      skip: (amountInStep) =>
        sdk.tokens.ROOT.hasEnoughAllowance(sdk.contracts.pipeline.address, sdk.contracts.hooliganhorde.address, amountInStep)
    }
  );
  ...
  clubhouseFarm.add(pipe);

  ////////// Estimate amountOut (number of ROOT) //////////

  console.log("\n\nEstimating...");
  const amountIn = amount.toBigNumber();
  const amountOut = await clubhouseFarm.estimate(amountIn);
  ...
  try {
    const txn = await clubhouseFarm.execute(amountIn, {
      slippage: 0.1, // verify
...
(async () => {
  //await (await (sdk.tokens.USDC as ERC20Token).approve(sdk.contracts.hooliganhorde.address, sdk.tokens.USDC.amount(101).toBigNumber())).wait();
  const tokenIn = sdk.tokens.HOOLIGAN;
  const amountIn = tokenIn.amount(100);

  // await chain.setUSDTBalance(account, amountIn);
  // await sdk.tokens.DAI.approve(sdk.contracts.hooliganhorde.address, tokenIn.amount(500).toBigNumber()).then((r) => r.wait());
  await test.setHOOLIGANBalance(account, amountIn);
  // await sdk.tokens.HOOLIGAN.approve(sdk.contracts.clubhouse.address, tokenIn.amount(500).toBigNumber()).then((r) => r.wait());

  console.log(`Approved and set initial balance to ${amountIn.toHuman()} ${tokenIn.symbol}.`);

  await roots_via_swap(tokenIn, sdk.contracts.clubhouse.address, amountIn);
})();
