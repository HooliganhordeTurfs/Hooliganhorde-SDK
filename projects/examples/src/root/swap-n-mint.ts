import { FarmFromMode, FarmToMode, TokenValue, Clipboard, Token, Workflow, ERC20Token, TestUtils } from "@hooligangturfs/sdk";
...
export async function roots_via_swap(inputToken: Token, amount: TokenValue) {
  ...
  // Swap from `inputToken` -> `depositToken` (HOOLIGAN)
  // If `swapDestination = INTERNAL`, and this is called via `hooliganhorde.farm()`,
  ...
  farm.add(new sdk.farm.actions.PermitERC20((context) => context.data.permit), {
    onlyExecute: true,
    skip: (amountInStep) => inputToken.hasEnoughAllowance(account, sdk.contracts.hooliganhorde.address, amountInStep)
  });
  ...
  pipe.add(
    ...
    function approveHooligan(amountInStep) {
      return pipe.wrap(depositToken.getContract(), "approve", [sdk.contracts.hooliganhorde.address, ethers.constants.MaxUint256], amountInStep);
    },
    {
      skip: (amountInStep) =>
        sdk.tokens.HOOLIGAN.hasEnoughAllowance(sdk.contracts.pipeline.address, sdk.contracts.hooliganhorde.address, amountInStep)
    }
  );
  ...
  try {
    ...
    const ready = await inputToken.hasEnoughAllowance(account, sdk.contracts.hooliganhorde.address, amountIn);
    if (!ready) {
...
(async () => {
  ...
  await sdk.tokens.DAI.approve(sdk.contracts.hooliganhorde.address, amountIn.toBigNumber()).then((r) => r.wait());
  // await chain.setDAIBalance(account, amountIn);
