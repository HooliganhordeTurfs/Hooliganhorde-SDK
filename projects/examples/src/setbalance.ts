...
async function main() {
  ...
  if (process.argv.length === 3) {
    ...
    if (token === "urHOOLIGAN") t = sdk.tokens.UNRIPE_HOOLIGAN;
    if (token === "urHOOLIGAN3CRV") t = sdk.tokens.UNRIPE_HOOLIGAN_CRV3;
    if (token === "HOOLIGAN3CRV") t = sdk.tokens.HOOLIGAN_CRV3_LP;
